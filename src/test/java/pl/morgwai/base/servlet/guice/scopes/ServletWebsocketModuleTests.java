...
public class ServletWebsocketModuleTests extends EasyMockSupport {
	...
	public void testInjectingContextBinderWithAllTrackers() throws Exception {
		...
		assertNotNull("an instance of ContextBinder should be properly injected",
				component.ctxBinder);
		final var testRequestCtx =
				new ServletRequestContext(mockServletRequest, servletModule.ctxTracker);
		final Runnable testTask = () -> assertSame("testTask should be bound to testRequestCtx",
				testRequestCtx, servletModule.ctxTracker.getCurrentContext());
		testRequestCtx.executeWithinSelf(() -> component.bindToCtx(testTask)).run();
	}
	...
	void testWebsocketContextProviders(boolean httpSessionPresent) {
		...
		final var connectionProxy =
				new WebsocketConnectionProxy(mockWebsocketConnection, websocketModule.ctxTracker);
